<p align="center">
  <img src="assets/lucid-light.svg" width="96" height="96" alt="Lucid light logo" />
</p>
...
AI agent가 정보를 빠르게 생성할수록, 그 내용을 읽고 이해하고 검토하는 일이 더 중요해진다.

Lucid는 응답을 짧게 만드는 데 그치지 않고, 내용을 쉽게 파악할 수 있도록 설명 방식과 글의 구조를 다듬는다.

Lucid는 에이전트의 응답에 다음 원칙을 적용한다.

- 핵심과 전체 그림을 먼저 보여주고, 핵심 요소와 그 관계, 필요한 세부 정보를 이어서 설명한다.
- 어려운 개념은 쉬운 말과 구체적인 설명으로 풀어 쓴다.
- 의미와 정확성을 유지하면서 반복과 군더더기를 줄인다. 설명을 짧게 만들려고 이해에 필요한 내용을 생략하지 않는다.
...
의미를 더하지 않는 인사말과 반복은 덜어낸다. 압축보다 명확한 설명을 우선하고,
이해에 필요한 정보나 근거를 길이 때문에 생략하지 않는다.
...
복잡한 정보, 분석 결과, 기술 개념을 설명하거나 정리해 달라고 요청하면 에이전트가
